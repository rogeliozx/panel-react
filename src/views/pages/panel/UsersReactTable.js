...
class UsersReactTable extends React.Component {
  state = {
      data:[],
      querys:{},
      countrySelect:"_ALL_",
  ...
  requestPage = () => {
    ...
    fetch('/api/users', {
        ...
        .then(response => {
            ...
            let data = response.data.map((user, k1)=>{
              const link = `https://www.waze.com/user/editor/${user.userName}`
              const updatedOn = user.updatedOn_segments>user.updatedOn_venues?user.updatedOn_segments:user.updatedOn_venues
              return {
                id:user.id,
                rank:user.rank,
                segments:<Link to={`/admin/Segments/updatedBy/${user.id}`}>{user.segments}</Link>,
                venues:<Link to={`/admin/Venues/updatedBy/${user.id}`}>{user.venues}</Link>,
                updatedOn: (new Date(updatedOn)).toLocaleDateString(),
                userName:user.userName,
                link:link,
                _segments:user.segments,
                _venues:user.venues,
                actions: this.renderActions( k1, link, user, false, false),
              }
            })
            ...
            const headers = [
              {label:'id',key:'id'},
              {label:'rank',key:'rank'},
              {label:'segments',key:'_segments'},
              {label:'venues',key:'_venues'},
