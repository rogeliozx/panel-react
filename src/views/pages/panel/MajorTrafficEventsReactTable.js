...
class MajorTrafficEventsReactTable extends React.Component {
  state = {
      data:[],
      querys:{},
      countrySelect:"_ALL_",
  ...
  componentDidMount(){
	...
	getFilters = () => {
    fetch('/api/majortrafficevents/filters')
    .then(res => res.json())
    .then(filters => {
      console.log(filters)
  ...
  substateSelected = ({target:{value:substate}}) => {
	...
	getQuerys = () => {
    fetch('/api/majortrafficevents/querys')
    .then(res => res.json())
    .then(querys => {
      console.log(querys)
  ...
  requestPage = () => {
    ...
    fetch('/api/majortrafficevents', {
        ...
        .then(response => {
            ...
            let data = response.data.map((trafic, k1)=>{
              let ENV = "row"
              switch (trafic.Env) {
                  case 'na':
                      ENV = "usa"
              ...
              // debugger
              let link =''
              const { city:[city={}]=[]} =trafic 
              const {centroid=null}= city
              if(centroid===null){
                link = `https://www.waze.com/editor/?env=${ENV}&zoom=4&majorTrafficEvent=${trafic._id}&mode=1`//&lat=20.511215660922304&lon=-103.39984004238575
              }else{
                const { coordinates:[lat,lon,]=[]} = centroid
                link = `https://www.waze.com/editor/?env=${ENV}&lon=${lat}&lat=${lon}&zoom=4&majorTrafficEvent=${trafic._id}&mode=1`//&lat=20.511215660922304&lon=-103.39984004238575

              }
              const {roadClosures=[], partners_docs=[], updatedOn=null} = trafic
              return {
                _id : trafic._id,
                category : EVENT_CATEGORIES[trafic.category],
                cityID : trafic._WZcityName,
                createdBy : trafic._createdBy,
                createdOn : (new Date(trafic.createdOn).toLocaleDateString()),
                startDate : trafic.startDate,
                endDate : trafic.endDate,
                names : trafic.names.filter(n=>n.locale==='en').map(n=>n.value).reduce((vi, va)=>`${vi}, ${va}`),
                partners : trafic.partners,
                updatedOn : updatedOn !== null ? (new Date(updatedOn).toLocaleDateString()) :'',
                partners : partners_docs.map(p=>p.userName).reduce((vi, va)=>`${vi} ${va}`,''),
                roadClosures:<Link to={`/admin/roadClosures/eventId/${trafic._id}`}>{roadClosures.length}</Link>,
                link:link,
                actions: this.renderActions( k1, link, trafic, false, false),
            }
            })
...
export default MajorTrafficEventsReactTable;
