...
class SegmentsReactTable extends React.Component {
  state = {
      data:[],
      querys:{},
      countrySelect:"_ALL_",
  ...
  requestPage = () => {
    ...
    fetch('/api/segments', {
        ...
        .then(response => {
            ...
            let data = response.data.map((segment, k1)=>{
              let ENV = "row"
              switch (segment.Env) {
                  case 'na':
                      ENV = "usa"
              ...
              // debugger
              var bounds = this.boundsGeometrysegment(segment.geometry);
              const lat = bounds.latMin+((bounds.latMax-bounds.latMin)/2);
              const lon = bounds.lonMin+((bounds.lonMax-bounds.lonMin)/2);
              const link = `https://www.waze.com/editor/?env=${ENV}&zoom=8&lon=${lat}&lat=${lon}&segments=${segment.id}&marker=true`
              return {
                id: segment._id,
                substate: segment._geoSubState,
                city:segment._WZcityName,
                name:segment._WZstreetName,
                rank:segment.rank,
                createdBy:segment._createdBy,
                createdOn:(new Date(segment.createdOn).toLocaleDateString()),
                updatedBy:segment._updatedBy,
                updatedOn:(new Date(segment.updatedOn).toLocaleDateString()),
                roadType:ROADTYPES[segment.roadType],
                link:link,
                actions: this.renderActions( k1, link, segment, false, false),
              }
            })
...
export default SegmentsReactTable;
