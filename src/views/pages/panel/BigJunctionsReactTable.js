...
class BigJunctionsReactTable extends React.Component {
  state = {
      data:[],
      querysSelect:"ALL",
  ...
  componentWillMount(){
	...
	getFilters = () => {
    fetch('/api/bigjunctions/filters')
    .then(res => res.json())
    .then(filters => {
      console.log(filters)
  ...
  requestPage = () => {
    ...
    fetch('/api/bigjunctions', {
        ...
        .then(response => {
            ...
            let data = response.data.map((junction, k1)=>{
              let ENV = "row"
              switch (junction.Env) {
                  case 'na':
                      ENV = "usa"
              ...
              var bounds = this.boundsGeometryVenue(junction.geometry);
              const lat = bounds.latMin+((bounds.latMax-bounds.latMin)/2);
              const lon = bounds.lonMin+((bounds.lonMax-bounds.lonMin)/2);
              
              const link = `https://www.waze.com/editor/?env=${ENV}&zoom=8&lon=${lat}&lat=${lon}&bigJunctions=${junction.id}&marker=true`
              return {
                id: junction._id,
                name:junction.name,
                City: junction.cityName,
                _area : (parseFloat(junction._area)).toFixed(2),
                segIDs : junction.segIDs.reduce((vi, va)=>`${vi}, ${va}`),
                createdBy:junction._createdBy,
                createdOn:(new Date(junction.createdOn).toLocaleDateString()),
                updatedBy:junction._updatedBy,
                updatedOn:(new Date(junction.updatedOn).toLocaleDateString()),
                link:link,
                actions: this.renderActions( k1, link, junction, false, false),
              }
            })
            const headers = [
              {label:'id',key:'id'},
              {label:'name',key:'name'},
              {label:'City',key:'City'},
              {label:'_area ',key:'_area '},
              {label:'segIDs ',key:'segIDs '},
          ...
          <Row>
            <Col md="12">
              <Card>
                <CardHeader>
                  <CardTitle tag="h4">
                    Big Junctions ( {this.state.data.length} ) 
                  </CardTitle>
                </CardHeader>
                <CardBody>
                <Row>
...
export default BigJunctionsReactTable;
